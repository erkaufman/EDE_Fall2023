...
#do we need to print an output here?
...
ggplot(Neonics) +
  geom_freqpoly(aes(x = Publication.Year), bins = 50) +
  geom_freqpoly(aes(x = Publication.Year), bins = 50, color = "magenta") +
  geom_freqpoly(aes(x = Publication.Year), bins = 50,  lty = 2)
...
```{r bar graph}
ggplot(Neonics) +
  geom_boxplot(aes(x = Endpoint)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
#what is this showing us? it looks alphabetircal..l am i doing it right?
```

> Answer: NR-LETH (Aquatic Terrestrial 100% mortality) and NR-ZERO (Aquatic Terrestrial 0% mortality)
...
Dates <- as.Date(Litter$collectDate) #convert the data within this column to class of dates

class(Dates) #checking the class of the new variable

unique(Dates) #getting the unique values within the vector

#do we just do this to a new variable, or to the column itself?
```
