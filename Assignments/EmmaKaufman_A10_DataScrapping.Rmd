...
scrape.it <- function(the_year, the_facility){
 ...
 #Set the element address variables
  ...
  Water_System <-  the_base_url %>% html_nodes(Water_System_tag) %>% html_text()
  PWSID <-  the_base_url %>% html_nodes(PWSID_tag) %>% html_text()
  Ownership <-  the_base_url %>% html_nodes(Ownership_tag) %>% html_text()
  MGD <-  the_base_url %>% html_nodes(MGD_tag) %>% html_text()
  Month<- the_base_url %>% html_nodes(Month_tag) %>% html_text()
...
ggplot(the_df,aes(x=Date,y=MGD)) + 
  ...
  labs(x = "Date",
       y = "Max Daily Withdrawal (MGD)",
       title= paste0("Maximum Daily Withdrawals across ", the_year))
