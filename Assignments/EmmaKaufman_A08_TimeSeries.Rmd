...
# 5
Days <-as.data.frame(seq(as.Date("2010-01-01"), as.Date("2019-12-31"), by = "days")) #creating dataframe
...
>Answer: It shows that ozone concentration has slightly decreased over time, it also shows yearly flucutations in ozone concentration. The slight decrease over time is a monotonic decreasing trend. 
...
> Answer: The seasonal Mann Kendall trend analysis is appropriate because our data shows strong seasonality.This is the only test that is appropriate for seasonal data, (unless we were to remove the seasonality from our data, in that case we could then try to run another test).  Additioanlly it is not linear, but still has a general monotonic trend (seen in the plot from question 7). 
Why is the seasonal Mann Kendall most appropriate? monotonic trend analysis, seasonality, non-parametric?
...
> Answer: The null hypothesis of the seasonal mann kendall is that the data are stationary. We get a p-value that is just below 0.05 (p= 0.04965), which means we can reject the null hypothesis and the trend we see in our data is statistically significant. When looking at each indivdual season we see that some individual seasons don't show the overall decreasing trend, but that the majority do have a large negative S value. We see this on our plot with the negative slope for the linear regression line in blue indicating a decrease in monthly mean ozone concentration over time. 
...
Garinger.late_Components <- as.data.frame(Monthly_Decomposed$time.series[,2:3]) #how to make this a timeseries? 

#16

Ozone_nonseasonal <- Kendall::SeasonalMannKendall(Garinger.late_Components) #how to interpret? 
