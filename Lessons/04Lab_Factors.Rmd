...
# knitr::opts_chunk$set(message = FALSE, warning = FALSE), alternative to the above
#Read packages
library(tidyverse);library(lubridate); library(here)

#here library will return a path where your project lives 

#Read in data, don't convert strings to factors
sites <- read.csv(here('Data/Raw/NWIS_SiteFlowData_NE_RAW.csv')) #or could separate by commas
dim(sites)

#Convert date column to date object
sites$date <- ymd_hms(sites$date) #refer to lubridate cheat sheet
...
summary(sites$dow)
#not all numbers are values, sometimes they are names
```
...
#Summary - is it meaningful?
summary(sites$dow) #different now that the column is a factor instead of numeric (each unique value is a grouping variable)
...
ggplot(sites,aes(x=dow)) +
  geom_bar() + 
  scale_x_discrete(drop=FALSE) #dont drop the levels that have no members
...
#Add **labels** to our factors, and write tidy code
sites$dow <- factor(
  sites$dow, 
...
#plot count plot of month column
