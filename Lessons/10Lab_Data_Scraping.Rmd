...
the_states_data <- the_website %>% html_nodes(the_states) %>% html_text()
the_price_data <- the_website %>%   html_nodes(the_price) %>%  html_text()
the_capacity_data <- the_website %>% html_nodes(the_capacity) %>% html_text()
net_generation_data <- the_website %>% html_nodes(net_generation) %>% html_text()
total_retail_data <- the_website %>% html_nodes(total_retail) %>% html_text()
  
#3 Construct a dataframe from the values
df <- data.frame(
  'state' = the_states_data,
  'price' = as.numeric(the_price_data),
  'Capacity' = as.numeric(gsub(',','',the_capacity_data)),
  'net generation' = as.numeric(gsub(',','',net_generation_data)),
  'total retail' = as.numeric(gsub(',','',total_retail_data))
)
