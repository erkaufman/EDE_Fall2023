...
2. Explore the components of times series: trend, seasonal, random/error, cyclical 
- allows us to make predicitons and forecast trends into the future, looks at behaviros of dataset in the past and tries to replicate that for future applications
- seasonality: winter/summer differences, weekend/weekday differences (assocaited with calendar events)
- cycle: up and downward movements, tend to be longer periods of duration, not related to seasonal or calendar events 
- cycle is represented with the trend (go up and down together)
- error term: random, can't model this component, want this to be uncorrelated (if its always positive or negative, then we are always either over or underestimating) need this to be random, shouldn't have correlation among random errors 
3. Learn how to perform stationarity test
- first few models rely on the idea that the data is statioinarity (mean, std deviation dont change over time)
- if yes/ if no determines which models are available for use 
- portfolio of models that can be run, but are only good for certain types of datasets
...
- sum up all the components, additive components 

cannot remove NA values, need to replace them with something else. You need data that is continuous and evenly spaced in order to run a good time series model 
...
- upward to downward movements in model
