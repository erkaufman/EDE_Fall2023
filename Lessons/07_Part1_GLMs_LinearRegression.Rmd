...
## Terminology
- response: variable we are trying to predict (dependent variable)
- independent variable: used to predict response (predictor)
- record: vector of predictors and outcome value from an observation
- intercept: when x is 0, what is y?
- regression coefficient: slope of the regression line
- fitted values: estimates of y obtained from the regression line (aka "predicition"), values of y we estimate with our model
- residuals: what is the difference between actual y and our predicted y 
- least squares: regression coefficient (minimum residuals)

##General workflow
- view data: scatterplot of y v. x, do you see a trend? 
- do you need to do a transformation to see a relationship? (log relationship?)
- if you see a trend, then create a linear model! 
    - find best fit line (lm() function in r, based on least squares)
    - assumes residuals are normal, sensitive to outliers!
    - assume causation (dependent)
- examine model summary to see if its a meaningful regression 
  - r2, p values (less than 0.05)
    - ex. 42% of iradiance in water is explained by depth, you could add more variables to get a better prediciton 
...
- want this plot to be as close to 0 as possible (very clustered around red lines means a good fit)
...
- examining the residuals (is the model good or not?), fitting a normal distribution to our residuals, if its a perfect normal distribution, all points will fall on the dotted line
...
- looking for trends in our results, want the red line to be as smooth as possible 
...
- important! leverage refers to the extent to which the point has impact on the model
- large residual, small leverage means its not affecting coefficients (need to remove it has large impact on the model?)
...
- explain third variable based on relationship of 2 could be overfitted
  - also need to look at relationships between indepedent variables 
  
- looks at AIC to choose best MLR; balance between adding more variables and how much it improves model fitting 
