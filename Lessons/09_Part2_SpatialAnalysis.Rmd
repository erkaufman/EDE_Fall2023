...
#
st_crs(counties_sf) #what is the coordinate reference system
...
#Create a map
ggplot() +
  geom_sf(data=huc8_sf, fill= 'orange')+ #want everything to be one color, don't need aesthetics
  geom_sf(data = huc8_sf_UN, fill = 'purple')

mapView(huc8_sf, col.region = 'orange') | #use the | in leaflet to get a side by side!
  mapView(huc8_sf_UN, col.region = 'turquoise')
...
huc_intersect <- huc8_sf_utm[tri_sf,] #not sure if this is correct

counties_intersect2 <- tri_sf %>% st_filter(huc8_sf_utm) #also not sure if this is correct 
