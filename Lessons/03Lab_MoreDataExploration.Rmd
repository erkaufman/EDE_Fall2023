...
#r setup is the name of the chunk, can't have the same name as another r code chunk
#ctrl alt i is inset chunk
#alt <- is insert assignment operator
#ctrl shift m is insert pipe operator
...
# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # sequence of numbers ranging from 1 to 20, with 5 rows and 4 columns
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #byrow or by column just changes how your data is organized within a matrix
matrix2
...
matrix1[4, ] #4th row of matrix, all columns
matrix1[ , 3] # all rows in the 3rd column
matrix1[c(12, 14)] #elements 12 and 14
matrix1[c(12:14)] #elements 12-14
matrix1[2:4, 1:3] #subset of rows and columns, defeault step of 1
...
high_temperatures <- c(90,95,87,101,115,99,98,100,102,92)
high_temperatures

low_temperatures <- c(55,55,57,61,61,58,60,55,54,53)
low_temperatures
```   
...
high_celsius= to_celsius(high_temperatures)
high_celsius

low_celsisus= to_celsius(low_temperatures)
low_celsisus
...
```{r DataExercise-1c}
Temp_dataframe = data.frame(low_temperatures,low_celsisus,high_temperatures,high_celsius)
names(Temp_dataframe) <- c("10 day low (F)","10 day low (C)","10 day high (F)", "10 day high (C)")
Temp_dataframe
```
...
```{r DataExercise-1d}
summary(Temp_dataframe)
sd(Temp_dataframe)
sd(Temp_dataframe$`10 day low (F)`)
summary(Temp_dataframe$`10 day low (F)`)
```
