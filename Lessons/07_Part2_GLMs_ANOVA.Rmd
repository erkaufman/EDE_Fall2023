...
- fitting linear models based on categorical variables, it's not continuous! 

##Terminology
- factor: independent variable used to group data; explains variability in another response variable 
- assumptions: populations are normally distributed, variances are equal, observations are independent (small samples can impact this, but you could still run the model)
...
- group data by factor, checked the normality assumptions, checked equal variance (not true)
- still ran ANOVA ('AOV')
  - lm on categorical variables automatically changes to AOV
    - it just changes the summary results (more in depth analysis avaiable with lm funciton)
...
- more than one variable or interactive effects 
  - this level from functional and class groups leads to something... maybe need to dive deeper into that? 
```{r}
# Format as aov
...
- post hoc tests: could you get a better model with different groupings 
