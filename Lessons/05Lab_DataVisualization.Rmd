...
ggplot(data=data.frame(x=0,y=0))+
  ...
  #add text to plot with geom_text()
  #specify the position on the x and y axis where you want the text to be displayed 
  #label argument is an expression in this case, using function paste() to "glue" together all arguments into one expression
  #could also use paste 
  geom_text(
    x=0,
    y=0.025,
...
#What kind of values are months? integers
class(PeterPaul.chem.nutrients$month)

#List unique values
unique(PeterPaul.chem.nutrients$month)
#see we don't have observations in every month
...
#use factor function
#month.abb gives you all 12 months in abbreviated format
factor(PeterPaul.chem.nutrients$month, levels=1:12, labels = month.abb) #could do this just for the plotting purpose within ggplot
...
ggplot(PeterPaul.chem.nutrients, 
  ...
  geom_bar()+
  scale_x_discrete(name= "month", drop = FALSE) #drop = FALSE means all factors will be printed
...
surface_temp_1999_plot + 
  ...
  #changing the name of the legend with the labs() function 
  labs(color='Lake') + 
  theme_wsj()
```
...
#can create a theme to make our own plots
```
...
surfacetemp<-
  ggplot(filter(PeterPaul.chem.nutrients, depth == 0),
         aes(
           x = daynum, 
           y= temperature_C, 
           color= year4 #why doesnt this have to be a factor (just depends on whether you have continuous or missing data, could do as.factor() if you want)
           ))+
  geom_point() +
